...
if [ -z "${MODEL}" ]; then
  ...
  echo "[v2t]   Run the selected skill's scripts/preflight.sh to fetch ggml-base.bin." >&2
  exit 1
fi
  ...
  if [ "$TRANSCRIBER" != "local" ] && { [ "$TRANSCRIBER" = "api" ] || asr_api_key_available; }; then
    if [ "$PROVIDER" = "siliconflow" ]; then
      ...
      if transcribe_via_siliconflow "$job_dir" "$srt_out"; then
        api_used=1
      elif [ "$TRANSCRIBER" = "api" ]; then
        fail_step asr "SiliconFlow ASR failed (see log above)"
        return 1
      else
        echo "[v2t] SiliconFlow ASR failed, falling back to local whisper" >&2
      fi
    else
