...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="最低点赞门槛（抖音建议 5000，小红书 1000），低于此值剔除",
    )
    p.add_argument(
        "--contents",
        default="",
        help="search_contents jsonl；默认取 MediaCrawler 当天文件",
    )
    p.add_argument(
        "--comments",
        default="",
    ...
    p.add_argument(
        "--out",
        default="",
        help="输出 markdown；daily 默认 爆款口播候选清单_YYYY-MM-DD.md，collect 默认 爆款口播候选清单_全量_YYYY-MM-DD.md",
    )
...
def default_jsonl(kind: str) -> Path:
    today = dt.date.today().isoformat()
    return Path.home() / "MediaCrawler" / "data" / "douyin" / "jsonl" / f"{kind}_{today}.jsonl"
...
def publish_dt(v: dict) -> dt.datetime | None:
...
def dedupe(videos: list[dict]) -> list[dict]:
    best: dict[str, dict] = {}
    for v in videos:
        aid = str(v.get("aweme_id") or v.get("aweme_url") or "")
        if not aid:
            continue
        cur = best.get(aid)
...
def top_comments(comments: list[dict], aweme_id: str, n: int = 3) -> list[dict]:
    items = [c for c in comments if str(c.get("aweme_id")) == str(aweme_id)]
...
def render(
    ...
    for c in comments:
        cmap[str(c.get("aweme_id"))].append(c)
    ...
    for name in ("绝对爆款", "中等热度", "中小热度"):
        ...
        for v in rows:
            ...
            lines.append(
                ...
                f"@{nick} · {pub_s} {age} · #{kw} · {hint}"
            )
            for c in top_comments(comments, str(v.get("aweme_id"))):
...
def main() -> None:
    args = parse_args()
    contents_path = Path(args.contents) if args.contents else default_jsonl("search_contents")
    comments_path = Path(args.comments) if args.comments else default_jsonl("search_comments")
    today = dt.date.today().isoformat()
    if args.out:
        out_path = Path(args.out)
    elif args.mode == "collect":
        out_path = Path.cwd() / f"爆款口播候选清单_全量_{today}.md"
    else:
        out_path = Path.cwd() / f"爆款口播候选清单_{today}.md"

    videos = dedupe(load_jsonl(contents_path))
    ...
    # 赞数门槛（用户 2026-08-14 定：抖音 ≥5000，小红书 ≥1000；低于直接剔除）
    if args.min_likes > 0:
        kept = []
        for v in videos:
            if to_int(v.get("liked_count")) < args.min_likes:
                continue
            kept.append(v)
        videos = kept
